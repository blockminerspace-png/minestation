...
export const AdminBlackMarket: React.FC<AdminBlackMarketProps> = ({ gameUpgrades = [] }) => {
    ...
    const [searchTerm, setSearchTerm] = useState('');
    const [newTax, setNewTax] = useState<string>('');
    const [newPriceBand, setNewPriceBand] = useState<string>('20');
    ...
    const loadData = async () => {
        ...
        setSettings(s);
        setNewTax((s?.marketTaxPercent ?? 0).toString());
        setNewPriceBand((s?.blackMarketPriceBandPercent ?? 20).toString());
        setLoading(false);
    };

    const handleSaveP2pPolicy = async () => {
        if (!settings) return;
        const tax = parseFloat(newTax);
        const band = parseFloat(newPriceBand);
        if (isNaN(tax) || tax < 0 || tax > 100) {
            alert("A taxa deve ser entre 0 e 100%");
            return;
        }
        if (isNaN(band) || band < 1 || band > 90) {
            alert("A faixa de preço P2P deve ser entre 1 e 90% (desvio máximo vs referência).");
            return;
        }
        await saveEconomySettings({ ...settings, marketTaxPercent: tax, blackMarketPriceBandPercent: band });
        alert("Política P2P atualizada com sucesso!");
        loadData();
    };
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="bg-slate-800 rounded-lg p-6 border border-slate-700 shadow-xl flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-white mb-2">Gestão P2P (Mercado Negro)</h2>
                    <p className="text-slate-400 text-sm mb-4">Monitore listagens, taxa sobre vendas e a faixa de preço permitida nas ofertas (vs referência de mercado).</p>
                ...
                <div className="bg-slate-900 p-4 rounded-lg border border-slate-700 flex flex-col sm:flex-row sm:items-end gap-4">
                    <div className="flex flex-col">
                        <label className="text-xs text-slate-500 font-bold uppercase mb-1">Taxa de Mercado (%)</label>
                        <div className="flex items-center gap-2">
                            <input
                    ...
                    <div className="flex flex-col flex-1 min-w-0">
                        <label className="text-xs text-slate-500 font-bold uppercase mb-1">Faixa de preço P2P (±%)</label>
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            />
                            <span className="text-slate-400">%</span>
                            <span className="text-[10px] text-slate-500 leading-tight">Preço permitido entre referência × (1 − %) e referência × (1 + %). Referência: última oferta ativa do item ou preço base da loja.</span>
                        </div>
                    </div>
                    <button
                        onClick={handleSaveP2pPolicy}
                        className="bg-red-600 hover:bg-red-500 text-white p-2 rounded transition-colors shrink-0"
                        title="Salvar taxa e faixa P2P"
                    >
                        <Save size={20} />
                    </button>
                </div>
