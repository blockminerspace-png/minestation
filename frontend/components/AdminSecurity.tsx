...
=======
import React, { useState, useEffect } from 'react';
import { SecurityStats, AdminDeviceFingerprintLog } from '../types';
import { getSecurityStats, addToBlacklist, removeFromBlacklist, getAdminDeviceFingerprints } from '../services/api';
import { Shield, AlertTriangle, Users, Globe, RefreshCw, Lock, Terminal, Ban, CheckCircle, Trash2, Fingerprint, Eye } from 'lucide-react';

export const AdminSecurity: React.FC = () => {
    const [stats, setStats] = useState<SecurityStats | null>(null);
    const [loading, setLoading] = useState(true);
    const [lastRefresh, setLastRefresh] = useState<Date>(new Date());
    const [activeTab, setActiveTab] = useState<'multi' | 'logs' | 'blacklist' | 'fingerprints'>('multi');
    const [banningIp, setBanningIp] = useState<string | null>(null);
