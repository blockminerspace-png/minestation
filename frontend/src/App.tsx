...
import { getBatteryUpgradeForRack } from './lib/rackBatteryResolve';
...
import { UI_PLACEHOLDER_ADS, FAVICON_PNG_URL } from './constants/assetPaths';
...
const calculateProduction = (
  ...
  placedRacks.forEach(rack => {
    const battery = getBatteryUpgradeForRack(rack, upgradesList, stored, workshopSlots);
...
const processLoadedState = (parsed: any, userEmail: string): GameState => {
  ...
  if (state.placedRacks) {
    state.placedRacks = state.placedRacks.map((r: any) => {
      ...
      let multiSlots = Array.isArray(r.multiplierSlots) ? r.multiplierSlots : [];
...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<View>(() => {
    try {
      ...
      if (saved && isGamePathView(saved)) return saved as View;
    } catch {
      /* ignore */
    }
    return 'servers';
  });
  /** Uma vez por entrada no jogo: aplicar vista a partir da URL em inglês. */
  ...
  /** Email + papel admin: evita recarregar o guardado / fechar o WS só porque `setUser({...user})` trocou a referência (ex. perfil). */
  const gameSaveLoadKey = user?.email?.trim() || '';
  ...
  useEffect(() => {
    if (globalView === 'game') {
      trackSpaPageView(gamePathFromView(currentView), `Genesis Miner — ${currentView}`);
    } else {
  ...
  /** Rotação dos skyscrapers: após `duration` (s) do anúncio visível à esquerda, passa ao próximo na lista. */
  const [verticalAdSlot, setVerticalAdSlot] = useState(0);
  ...
  /** Base URL pública (FRONTEND_URL / CORS no servidor); link de referral quando ≠ host da página. */
  const [appPublicOrigin, setAppPublicOrigin] = useState<string | null>(null);
  ...
  /** Barra de endereço em inglês (`/servers`, `/wallet`, …) alinhada ao ecrã do jogo. */
  useEffect(() => {
    if (globalView !== 'game' || !user || user.isAdmin) return;
    const next = gamePathFromView(currentView);
  ...
  }, [user, user?.isAdmin, globalView, saveLoaded, accessLevels, economySettings, isOperatorAdminOnly]);

  useEffect(() => {
    if (!saveLoaded || !user || user.isAdmin || globalView !== 'game') return;
    ...
    (async () => {
      try {
        ...
        if (boot) {
          ...
          setAppPublicOrigin(boot.appOrigin);
          return;
        }
  ...
  useEffect(() => {
    const n = verticalAds.length;
    if (n === 0) {
      setVerticalAdSlot(0);
      return;
    }
    setVerticalAdSlot((s) => s % n);
  }, [verticalAds]);

  useEffect(() => {
    const n = verticalAds.length;
    if (n === 0) return;
    const idx = ((verticalAdSlot % n) + n) % n;
    const rawDur = Number(verticalAds[idx]?.duration);
    const sec =
      Number.isFinite(rawDur) && rawDur >= 1 ? Math.min(3600, Math.floor(rawDur)) : 60;
    const ms = Math.max(3000, sec * 1000);
    const t = window.setTimeout(() => {
      setVerticalAdSlot((s) => (s + 1) % n);
    }, ms);
    return () => clearTimeout(t);
  }, [verticalAds, verticalAdSlot]);

  const { verticalLeftAd, verticalRightAd } = useMemo(() => {
    const n = verticalAds.length;
    if (n === 0) {
      return { verticalLeftAd: null as SystemNews | null, verticalRightAd: null as SystemNews | null };
    }
    const left = verticalAds[verticalAdSlot % n];
    const right = n > 1 ? verticalAds[(verticalAdSlot + 1) % n] : left;
    return { verticalLeftAd: left, verticalRightAd: right };
  }, [verticalAds, verticalAdSlot]);

  // WebSocket: cabeçalho do jogo — saldos (coin_balances + USDC) e hashrate alinhados à BD (~3,5s)
  useEffect(() => {
    if (!gameSaveLoadKey || gameSaveLoadIsAdmin || globalView !== 'game' || !saveLoaded) {
  ...
  }, [gameSaveLoadKey, gameSaveLoadIsAdmin, globalView, saveLoaded]);
    ...
    (async () => {
      try {
        ...
        if (data) {
          ...
          const parsed = processLoadedState(data, gameSaveLoadKey);
          setGameState(parsed);
          setSaveLoaded(true);
          setGameStateLoadError(null);
        } else if (status === 404) {
          setGameState(INITIAL_STATE);
          await apiSaveGameState(gameSaveLoadKey, INITIAL_STATE);
          if (!cancelled) {
            setSaveLoaded(true);
            setGameStateLoadError(null);
          }
  ...
  }, [gameSaveLoadKey, gameSaveLoadIsAdmin, gameStateReloadNonce]);
  ...
  useEffect(() => {
    setProductionRate(
      calculateProduction(
        ...
        gameUpgrades,
        gameState.storedBatteries || [],
        gameState.workshopSlots
      )
    );
  }, [gameState.placedRacks, gameUpgrades, gameState.storedBatteries, gameState.workshopSlots]);
    ...
    const interval = setInterval(() => {
      setGameState(prev => {
        ...
        const nextRacks = prev.placedRacks.map(r => {
          const batt = getBatteryUpgradeForRack(r, gameUpgrades, prev.storedBatteries || [], prev.workshopSlots);
    ...
    const tick = async () => {
      ...
      try {
        ...
        if (lite) {
          ...
          if (lite.appOrigin) setAppPublicOrigin(lite.appOrigin);
          return;
        }
  ...
  const handleUpdateUser = async (updatedUser: User & { currentPassword?: string }) => {
    const r = await apiUpdateUser(updatedUser);
    if (!r.ok) {
      return r;
    }
    const safe = { ...updatedUser } as Record<string, unknown>;
    delete safe.password;
    delete safe.currentPassword;
    setUser(safe as unknown as User);
    return r;
  };
  ...
  const handleUpgradeAccess = (newLevelId: string) => {
    ...
    void (async () => {
      const r = await handleUpdateUser(updatedUser);
      if (!r.ok) return;
  ...
  const handleRemoveRack = useCallback((rackId: string) => {
    ...
    setGameState(p => {
      ...
      if (r.batteryId) {
        const ret = applyRackBatteryUnequipToInventory(r.batteryId, r.currentCharge, nb, ns, gameUpgrades);
        nb = ret.nextNb;
        Object.assign(ns, ret.nextNs);
      }
  ...
  const handleEquipAux = useCallback((rid: string, iid: string, type: string, sbid?: string, idx?: number) => {
    setGameState(p => {
      ...
      if (oldItemId) {
        if (type === 'battery') {
          const ret = applyRackBatteryUnequipToInventory(oldItemId, oldCharge, nb, ns, gameUpgrades);
          nb = ret.nextNb;
          Object.assign(ns, ret.nextNs);
        } else {
      ...
      if (type === 'battery') {
        if (sbid) {
          ...
          if (!s) return p;
          initCharge = typeof s.currentCharge === 'number' ? s.currentCharge : gameUpgrades.find((u) => u.id === iid)?.powerCapacity || 0;
        } else {
          if ((ns[iid] || 0) < 1) return p;
          ns[iid]--;
          initCharge = gameUpgrades.find((u) => u.id === iid)?.powerCapacity || 0;
        }
        r.batteryId = sbid ? sbid : iid;
  ...
  const handleUnequipAux = useCallback((rid: string, type: string, idx?: number) => {
    setGameState(p => {
      ...
      if (type === 'battery') {
        const ret = applyRackBatteryUnequipToInventory(id, r.currentCharge, nb, ns, gameUpgrades);
        nb = ret.nextNb;
        Object.assign(ns, ret.nextNs);
  ...
  const handleRecharge = useCallback((rid: string) => {
    setGameState((p) => {
      ...
      const def = getBatteryUpgradeForRack(r, gameUpgrades, p.storedBatteries || [], p.workshopSlots);
      ...
      if (!res.ok) {
        if (res.forceReload) {
          ...
          setBulkBatteryNotice({
            title: 'Moeda na sala',
            message: 'O servidor recusou o pedido com estado incompleto. O jogo foi sincronizado com a BD.'
          });
          return;
        }
  ...
  const handleSetRoomRacksBattery = useCallback(
    async (roomId: string, batteryUpgradeId: string, opts?: BulkRoomBatteryRunOptions) => {
      const roomNorm = normalizePlacedRackRoomId(roomId);
        ...
        <div className="max-w-7xl mx-auto min-w-0 w-full px-4 py-2 md:py-2.5 flex flex-col md:flex-row justify-between items-center gap-2 md:gap-3">
          ...
          >
            <div className="w-10 h-10 rounded-full overflow-hidden shrink-0 ring-2 ring-amber-500/50 shadow-lg shadow-amber-600/25 bg-slate-900">
              <img src={FAVICON_PNG_URL} alt="" className="w-full h-full object-cover" width={40} height={40} fetchPriority="high" aria-hidden />
            </div>
            <div>
                    ...
                    (() => {
                      ...
                      const coinsWithPower = miningCoins.map((c) => {
                        ...
                        gameState.placedRacks.forEach(r => {
                          const batt = getBatteryUpgradeForRack(
                            r,
                            gameUpgrades,
                            gameState.storedBatteries || [],
                            gameState.workshopSlots
          ...
          <>
            ...
            <div className="flex-1 min-w-0 flex justify-center overflow-hidden relative w-full h-full">
              ...
              <aside className="hidden 2xl:flex shrink-0 w-[145.6px] h-[546px] sticky top-24 mx-4 overflow-hidden rounded-xl border border-amber-500/20 bg-slate-900/40 backdrop-blur-sm self-start mt-4 transition-all duration-500 hover:border-amber-500/40 shadow-2xl shadow-amber-500/5">
                {verticalLeftAd ? (
                  <a href={verticalLeftAd.link || '#'} target={verticalLeftAd.link ? "_blank" : "_self"} rel="noopener noreferrer" className="w-full h-full block">
                    {verticalLeftAd.imageUrl ? (
                      <RemoteBannerImage
                        src={verticalLeftAd.imageUrl}
                        alt={verticalLeftAd.text}
                      ...
                      <div className="w-full h-full flex items-center justify-center p-4 text-center bg-slate-950/50">
                        <span className="text-xs text-amber-400 font-bold uppercase">{verticalLeftAd.text}</span>
                      </div>
                    )}
                  </a>
                ) : (
                  <RemoteBannerImage
                    src={UI_PLACEHOLDER_ADS.skyLeft}
                      ...
                      <div className="flex-1">
                        <Suspense fallback={<LazyRouteFallback />}>
                          <ProfilePage
                            ...
                            onUpdateGameState={(next) => setGameState(next)}
                            referralBaseUrl={appPublicOrigin}
                          />
                        </Suspense>
                      </div>
                      <Footer />
              ...
              <aside className="hidden 2xl:flex shrink-0 w-[145.6px] h-[546px] sticky top-24 mx-4 overflow-hidden rounded-xl border border-orange-500/20 bg-slate-900/40 backdrop-blur-sm self-start mt-4 transition-all duration-500 hover:border-orange-500/40 shadow-2xl shadow-orange-500/5">
                {verticalRightAd ? (
                  <a href={verticalRightAd.link || '#'} target={verticalRightAd.link ? "_blank" : "_self"} rel="noopener noreferrer" className="w-full h-full block">
                    {verticalRightAd.imageUrl ? (
                      <RemoteBannerImage
                        src={verticalRightAd.imageUrl}
                        alt={verticalRightAd.text}
                        className="w-full h-full object-contain"
                        failureHint="Lateral — URL falhou"
                      />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center p-4 text-center bg-slate-950/50">
                        <span className="text-xs text-orange-400 font-bold uppercase">{verticalRightAd.text}</span>
                      </div>
                    )}
                  </a>
                ) : (
                  <RemoteBannerImage
                    src={UI_PLACEHOLDER_ADS.skyRight}
                    alt="Lateral Direita"
                    className="w-full h-full object-contain"
                    failureHint="Sem imagem"
                  />
