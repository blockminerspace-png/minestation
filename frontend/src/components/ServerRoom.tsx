...
import { getBatteryUpgradeForRack, resolveRackBatteryItemIdWithWorkshopFallback } from '../lib/rackBatteryResolve';
import {
...
import {
  getMyRigRooms,
  purchaseRoomSlot,
  getServerRoomBulkBatteriesPreview,
  type ServerRoomBulkBatteriesPreviewSuccess
} from '../services/api';
import type { BulkRoomBatteryRunOptions } from '../types/bulkRoomBattery';
import { MiningCoinSelect } from './MiningCoinSelect';
import { Server, XCircle, Zap, Power, Plus, Cog, X, Box, Save, Activity, Calculator, Coins, Battery, Cpu } from 'lucide-react';
...
interface ServerRoomProps {
    ...
    /** Resolve `battery_id` polimórfico (catálogo vs instância) com fallback à oficina. */
...
/** Ícone quando a imagem do componente falha ou URL está vazia. */
function RackSlotVisualFallback({ item }: { item: Upgrade | undefined }) {
    const cls = 'text-amber-500/90 drop-shadow-[0_0_6px_rgba(251,191,36,0.35)]';
    const t = item?.type;
    if (t === 'battery') return <Battery className={cls} size={26} aria-hidden />;
    if (t === 'wiring') return <Zap className={cls} size={26} aria-hidden />;
    if (t === 'multiplier') return <Activity className={cls} size={26} aria-hidden />;
    return <Cpu className={cls} size={26} aria-hidden />;
}
...
const AnimatedMiner = ({
    ...
    useEffect(() => {
        ...
        probe.onload = () => {
            if (!cancelled) setMediaOk(true);
        };
        probe.onerror = () => {
            if (!cancelled) setMediaOk(false);
        };
        probe.src = src;
        return () => {
            cancelled = true;
    ...
    useEffect(() => {
        if (!mediaOk || !src) return;
        if (!isOperational && src && !staticImage) {
            const img = new Image();
            img.crossOrigin = 'anonymous';
            img.onerror = () => setStaticImage(null);
            img.onload = () => {
    ...
    }, [isOperational, src, staticImage, mediaOk]);

    if (!src || !mediaOk) {
        return (
            <div
                className={`${className} flex items-center justify-center bg-slate-900/88`}
                style={style}
                title="Imagem indisponível"
            >
                <RackSlotVisualFallback item={item} />
            </div>
        );
    }
...
export const ServerRoom: React.FC<ServerRoomProps> = ({
    ...
    type ServerRoomDetailContext =
        | { mode: 'item'; rackId: string; slotIndex: number | null; type: 'machine' | 'battery' | 'wiring' | 'multiplier'; item: Upgrade }
        | { mode: 'orphan-battery'; rackId: string; slotIndex: number | null };
    const [detailContext, setDetailContext] = useState<ServerRoomDetailContext | null>(null);
    ...
    type RoomBulkBattPrevState =
        | { status: 'idle' }
        | { status: 'loading' }
        | { status: 'ready'; data: ServerRoomBulkBatteriesPreviewSuccess }
        | { status: 'error'; message: string };
    const [roomBulkBattPrev, setRoomBulkBattPrev] = useState<RoomBulkBattPrevState>({ status: 'idle' });
    ...
    const roomTotalProduction = useMemo(() => {
        return calculatePlacedRacksProductionHashrate(currentRoomRacks, upgrades, storedBatteries, workshopSlots);
    }, [currentRoomRacks, upgrades, storedBatteries, workshopSlots]);
    ...
    const handleSlotClick = (rackId: string | null, slotIndex: number, currentItemId: string | null, isRoomSlot: boolean = false) => {
        ...
        if (currentItemId) {
            ...
            if (item) setDetailContext({ mode: 'item', rackId, slotIndex, type: 'machine', item });
        } else {
    ...
    const handleAuxClick = (rackId: string, currentItemId: string | null, type: 'battery' | 'wiring' | 'multiplier', slotIndex?: number) => {
        const si = slotIndex ?? null;
        if (type === 'battery' && currentItemId) {
            const catalogId = resolveRackBatteryItemIdWithWorkshopFallback(
                currentItemId,
                upgrades,
                storedBatteries,
                workshopSlots
            );
            const item = catalogId ? upgrades.find((u) => u.id === catalogId) : undefined;
            if (item) {
                setDetailContext({ mode: 'item', rackId, slotIndex: si, type: 'battery', item });
                return;
            }
            setDetailContext({ mode: 'orphan-battery', rackId, slotIndex: si });
            return;
        }
        if (currentItemId) {
            const item = upgrades.find(u => u.id === currentItemId);
            if (item) setDetailContext({ mode: 'item', rackId, slotIndex: si, type, item });
        } else {
            setSelectionContext({ rackId, slotIndex: si, type });
        }
    };
    ...
    const openRoomBulkBatteryModal = (room: RigRoom) => {
        ...
        const racksHere = placedRacks.filter((r) => sameRigRoom(r.roomId, room.id));
        if (racksHere.length === 0) return;
        const rawBatt = racksHere.find((r) => r.batteryId)?.batteryId ?? '';
        const itemBatt =
          resolveRackBatteryItemIdWithWorkshopFallback(rawBatt, upgrades, storedBatteries, workshopSlots) || rawBatt;
        const canPickManual = itemBatt && (stock[itemBatt] || 0) > 0;
        setRoomBulkBatterySelect(canPickManual ? itemBatt : '');
        setRoomBulkBatterySmartFill(false);
        setRoomBulkBatteryRigSort('slot_asc');
        setRoomBulkBattPrev({ status: 'loading' });
    ...
    const closeRoomBulkBatteryModal = () => {
        ...
        setRoomBulkBatterySmartFill(false);
        setRoomBulkBatteryRigSort('slot_asc');
        setRoomBulkBattPrev({ status: 'idle' });
    };

    useEffect(() => {
        if (!roomBulkBatteryModal || !onSetRoomRacksBattery) {
            setRoomBulkBattPrev({ status: 'idle' });
            return;
        }
        const ac = new AbortController();
        const roomIdNorm = normalizePlacedRackRoomId(roomBulkBatteryModal.id);
        setRoomBulkBattPrev({ status: 'loading' });
        const t = window.setTimeout(() => {
            void getServerRoomBulkBatteriesPreview({
                roomId: roomIdNorm,
                rigSort: roomBulkBatteryRigSort,
                smartFill: roomBulkBatterySmartFill,
                batteryUpgradeId: roomBulkBatterySmartFill ? '' : roomBulkBatterySelect,
                signal: ac.signal
            }).then((res) => {
                if (ac.signal.aborted) return;
                if (res.ok === true) {
                    setRoomBulkBattPrev({ status: 'ready', data: res });
                    return;
                }
                if (res.error === '__abort__') return;
                setRoomBulkBattPrev({ status: 'error', message: res.error });
            });
        }, 280);
        return () => {
        ...
        roomBulkBatteryModal,
        roomBulkBatteryRigSort,
        ...
        <div className="flex flex-col gap-6 relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12 justify-items-center p-4">
                {Array.from({ length: roomCapacity }).map((_, slotIdx) => {
                    ...
                    const battery = getBatteryUpgradeForRack(rack, upgrades, storedBatteries, workshopSlots);
                                            ...
                                            <>
                                                ...
                                                <div className="absolute inset-0 z-10 p-2">
                                                    {layoutToUse.slots.map((slot, i) => {
                                                        ...
                                                        const item = upgradeLookupId ? upgrades.find((u) => u.id === upgradeLookupId) : null;
                                                        const itemImg = normalizePublicAssetUrl(item?.image);
                                                        const batterySlotOrphan =
                                                            slot.type === 'battery' && !!rack.batteryId && !upgradeLookupId;
                                                        ...
                                                        if (slot.type === 'power') {
                                                            ...
                                                            const batResolved = resolveRackBatteryItemIdWithWorkshopFallback(
                                                                rack.batteryId,
                                                                upgrades,
                                                                storedBatteries,
                                                                workshopSlots
                                                            );
                                                            if (!rack.batteryId || !batResolved) missing.push('Bateria');
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                title={
                                                                    batterySlotOrphan
                                                                        ? 'Referência de bateria inválida (dados em falta). Clique para reequipar.'
                                                                        : undefined
                                                                }
                                                            ...
                                                            >
                                                                ...
                                                                {item && !itemImg && (
                                                                    <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-slate-900/75">
                                                                        <RackSlotVisualFallback item={item} />
                                                                    </div>
                                                                )}
                        ...
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
                            ...
                            <div className="p-4 space-y-4">
                                {(() => {
                                    ...
                                    const battery = rack.batteryId
                                        ? getBatteryUpgradeForRack(rack, upgrades, storedBatteries, workshopSlots)
                                        : null;
                                    const batteryRefBroken =
                                        !!rack.batteryId &&
                                        !resolveRackBatteryItemIdWithWorkshopFallback(
                                            rack.batteryId,
                                            upgrades,
                                                ...
                                                <div className="bg-slate-100 dark:bg-slate-800 rounded p-3 border border-slate-200 dark:border-slate-700">
                                                    ...
                                                    {battery ? (
                                                        ...
                                                        <div className="text-xs text-amber-600 dark:text-amber-400">
                                                            Referência de bateria inválida ou em falta no armazém. Retire e volte a equipar uma bateria válida.
                                                        </div>
                                                    ) : (
                                                        <div className="text-xs text-slate-500">Nenhuma instalada.</div>
                                                    )}
            ...
            {
                detailContext && (() => {
                    if (detailContext.mode === 'orphan-battery') {
                        return (
                                ...
                                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
                                    <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                                        <h3 className="font-bold text-slate-800 dark:text-white text-sm flex items-center gap-2">
                                            <Battery className="text-amber-600 dark:text-amber-400 shrink-0" size={18} aria-hidden />
                                            Bateria (referência inválida)
                                        </h3>
                                        <button type="button" onClick={() => setDetailContext(null)} className="text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white">
                                            <X size={16} />
                                        </button>
                                    </div>
                                    <div className="p-4 space-y-3 text-sm text-slate-700 dark:text-slate-300">
                                        <p>
                                            O slot tem um ID de bateria que não corresponde ao catálogo nem ao armazém (dados dessincronizados ou instância em falta).
                                            Pode remover o slot para limpar a rig; o inventário válido não será alterado.
                                        </p>
                                    </div>
                                    <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex gap-2">
                                        <button type="button" onClick={() => setDetailContext(null)} className="bg-slate-700 text-white px-4 py-2 rounded font-bold">
                                            Fechar
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => {
                                                onUnequipAux(detailContext.rackId, 'battery', detailContext.slotIndex ?? undefined);
                    ...
                    const detailImg = normalizePublicAssetUrl(dc.item.image);
                    return (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 dark:bg-slate-950/80 backdrop-blur-sm">
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
                            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                            ...
                            <div className="p-4 space-y-3">
                                <div className="flex items-center gap-3">
                                    <div className="w-14 h-14 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 overflow-hidden flex items-center justify-center text-2xl">
                                        {detailImg ? (
                                            <img src={detailImg} className="w-full h-full object-cover" alt="" />
                                        ) : (
                                            dc.item.icon
                                        )}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="text-xs text-slate-500 dark:text-slate-400">{dc.item.category}</div>
                                        <div className="text-[10px] text-slate-400 dark:text-slate-500">ID: {dc.item.id}</div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-2 text-xs text-slate-600 dark:text-slate-400">
                                    {typeof dc.item.baseProduction === 'number' && dc.item.baseProduction > 0 && (
                                        <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded p-2">
                                            ...
                                            <div>{dc.item.baseProduction} N/s</div>
                                        </div>
                                    )}
                                    {typeof dc.item.powerConsumption === 'number' && dc.item.powerConsumption! > 0 && (
                                        <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded p-2">
                                            <div className="font-bold text-slate-800 dark:text-slate-200">Consumo</div>
                                            <div>{dc.item.powerConsumption} W</div>
                                        </div>
                                    )}
                                    {typeof dc.item.powerCapacity === 'number' && dc.item.powerCapacity! > 0 && (
                                        <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded p-2">
                                            <div className="font-bold text-slate-800 dark:text-slate-200">Capacidade</div>
                                            <div>{dc.item.powerCapacity} Wh</div>
                                        </div>
                                    )}
                                    {typeof dc.item.multiplier === 'number' && dc.item.multiplier! > 0 && (
                                        <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded p-2">
                                            <div className="font-bold text-slate-800 dark:text-slate-200">Multiplicador</div>
                                            <div>{(dc.item.multiplier * 100).toFixed(1)}%</div>
                                        </div>
                                    )}
                                </div>
                            </div>
                            <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex gap-2">
                                ...
                                <button
                                    type="button"
                                    onClick={() => {
                                        if (!detailContext || detailContext.mode !== 'item') return;
            ...
            {roomBulkBatteryModal && onSetRoomRacksBattery && (() => {
                const pv = roomBulkBattPrev.status === 'ready' ? roomBulkBattPrev.data : null;
                const previewOptionRows = new Map(
                    (pv?.batteryOptions ?? []).map((o) => [o.id, o] as const)
                );
                const selectedRow = roomBulkBatterySelect ? previewOptionRows.get(roomBulkBatterySelect) : undefined;
                const loadingPreview =
                    roomBulkBattPrev.status === 'loading' || roomBulkBattPrev.status === 'idle';
                const modalWillApply =
                    roomBulkBatterySelect && pv?.scenario?.ok ? Math.max(0, pv.scenario.appliedRigs || 0) : 0;
                const smartFillOk = pv?.smartFillOk === true;
                const scenarioOk = !!(pv && pv.scenario.ok);
                const applyDisabled = loadingPreview
                    ? true
                    : roomBulkBatterySmartFill
                      ? !smartFillOk
                      : roomBulkBatterySelect
                        ? !selectedRow || selectedRow.compatibleRigs === 0 || selectedRow.availableUnits < 1
                        : !scenarioOk;
                return (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 dark:bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl w-full max-w-lg max-h-[min(92vh,36rem)] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
                            ...
                            {roomBulkBattPrev.status === 'error' && (
                                <p className="border-b border-amber-200 bg-amber-50 px-4 py-2 text-xs text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/50 dark:text-amber-100">
                                    {roomBulkBattPrev.message}
                                </p>
                            )}
                                ...
                                <div className={roomBulkBatterySmartFill ? 'pointer-events-none opacity-50' : ''}>
                                    ...
                                    <div className="max-h-[min(52vh,20rem)] space-y-1.5 overflow-y-auto rounded-lg border border-slate-300 bg-slate-50 p-1.5 dark:border-slate-600 dark:bg-slate-950/80 custom-scrollbar">
                                        ...
                                        {upgrades
                                            ...
                                            .map((u) => {
                                                const row = previewOptionRows.get(u.id);
                                                const subtitle = loadingPreview
                                                    ? 'A calcular no servidor…'
                                                    : roomBulkBattPrev.status === 'error'
                                                      ? 'Pré-visualização indisponível.'
                                                      : row
                                                        ? row.compatibleRigs > 0
                                                          ? `Compatíveis: ${row.compatibleRigs} · disponível ${row.availableUnits} → até ${row.willApply} rig(s)`
                                                ...
                                                return (
                                                    <BatteryOptionRow
                                                        ...
                                                        disabled={!row || row.compatibleRigs === 0}
                                                        subtitle={subtitle}
                                                        onPick={() => {
                                                            setRoomBulkBatterySmartFill(false);
                            ...
                            <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex gap-2 justify-end bg-slate-50 dark:bg-slate-950">
                                ...
                                <button
                                    ...
                                    title={
                                        applyDisabled
                                            ? loadingPreview
                                                ? 'A carregar pré-visualização do servidor.'
                                                : roomBulkBatterySmartFill
                                                  ? pv?.smartFillMessage ||
