...
import { Users, Zap, Database, Activity, Clock, Trophy, DollarSign, Download, EyeOff, Eye, Construction } from 'lucide-react';
import {
    getGameState,
    getTopWithdrawalsByCoin,
    getMiningCoins,
...
interface AdminDashboardProps {
    ...
    /** Só super-admin vê o cartão de manutenção pública (genesisdao.tech). */
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ users, gameUpgrades, currentUser = null }) => {
    ...
    const [maintDraftActive, setMaintDraftActive] = useState(false);
    const [maintDraftMessage, setMaintDraftMessage] = useState('');
    const [maintLoaded, setMaintLoaded] = useState(false);
    const [maintBusy, setMaintBusy] = useState(false);
    const [maintError, setMaintError] = useState<string | null>(null);
    const [maintLastInvalidated, setMaintLastInvalidated] = useState<string | null>(null);
    ...
    useEffect(() => {
        ...
        (async () => {
            setMaintError(null);
            const s = await getAdminMaintenance();
            if (cancelled) return;
            if (!s) {
                setMaintLoaded(true);
                setMaintError('Não foi possível carregar o estado de manutenção.');
                return;
            }
            setMaintDraftActive(s.active);
            setMaintDraftMessage(s.message || '');
            setMaintLoaded(true);
        })();
        return () => {
            cancelled = true;
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            {currentUser?.isSuperAdmin && (
                <div className="rounded-xl border border-slate-600/80 bg-slate-800/60 px-4 py-4 text-sm text-slate-200">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        <div className="flex items-start gap-3 min-w-0">
                            <Construction className="text-amber-500 shrink-0 mt-0.5" size={22} aria-hidden />
                        ...
                        <label className="flex items-center gap-2 shrink-0 cursor-pointer select-none">
                            <input
                                ...
                                checked={maintDraftActive}
                                disabled={!maintLoaded || maintBusy}
                                onChange={(e) => setMaintDraftActive(e.target.checked)}
                            />
                            <span className="text-xs font-bold text-slate-300">Manutenção ativa</span>
                        </label>
                    </div>
                    <div className="mt-3 space-y-2">
                        <label className="block text-xs font-bold text-slate-400 uppercase tracking-wide">
                            Mensagem (opcional, guardada em settings)
                        </label>
                        <textarea
                            className="w-full min-h-[88px] rounded-lg border border-slate-600 bg-slate-900/80 px-3 py-2 text-xs font-mono text-slate-200 placeholder:text-slate-600 disabled:opacity-50"
                            placeholder="Texto livre (a página Nginx atual é estática; útil para APIs ou evolução futura)."
                            value={maintDraftMessage}
                            disabled={!maintLoaded || maintBusy}
                            onChange={(e) => setMaintDraftMessage(e.target.value)}
                        />
                    </div>
                    {maintError && (
                        <p className="mt-2 text-xs text-red-400" role="alert">{maintError}</p>
                    )}
                    {maintLastInvalidated && (
                        <p className="mt-2 text-xs text-emerald-400/90">{maintLastInvalidated}</p>
                    )}
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                        <button
                            type="button"
                            className="rounded-lg bg-amber-600 px-4 py-2 text-xs font-bold text-white hover:bg-amber-500 disabled:opacity-50 disabled:pointer-events-none"
                            disabled={!maintLoaded || maintBusy}
                            onClick={async () => {
                                setMaintBusy(true);
                                setMaintError(null);
                                setMaintLastInvalidated(null);
                                const r = await updateAdminMaintenance({
                                    active: maintDraftActive,
                                    message: maintDraftMessage,
                                });
                                setMaintBusy(false);
                                if (r.ok === false) {
                                    setMaintError(r.error);
                                    return;
                                }
                                setMaintDraftActive(r.active);
                                setMaintDraftMessage(r.message || '');
                                if (r.invalidated) {
                                    setMaintLastInvalidated(
                                        `Sessões invalidadas: ${r.invalidated.sessions} sessões, ${r.invalidated.jwtRefresh} refresh tokens.`
                                    );
                                }
                            }}
                        >
                            {maintBusy ? 'A guardar…' : 'Aplicar'}
                        </button>
                        {!maintLoaded && <span className="text-xs text-slate-500">A carregar estado…</span>}
                    </div>
                </div>
            )}
