...
import { resolveRackBatteryItemIdWithWorkshopFallback } from '../lib/rackBatteryResolve';
...
export const AdminUsers: React.FC<AdminUsersProps> = ({
    ...
    /** Admin a editar perfil de outro utilizador (não exige senha atual para nova senha). */
    ...
    const handleUpdateUserProfile = async () => {
        ...
        const payload: User = {
            ...
            polygonWallet: editProfileForm.wallet.trim() || null,
        ...
        if (editProfileForm.password && editProfileForm.password.trim().length > 0) {
            ...
            if (!editingOther) {
                const cp = editProfileForm.currentPassword?.trim() || '';
                if (!cp) {
                    alert('Indique a sua palavra-passe atual para alterar a sua própria senha.');
                    return;
                }
                (payload as User & { currentPassword?: string }).currentPassword = cp;
            }
        }
        ...
        setEditProfileForm((f) => ({ ...f, password: '', currentPassword: '' }));
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in slide-in-from-right-4">
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 h-fit">
                    ...
                    <div className="space-y-3">
                        ...
                        {!adminEditingOtherUser && editProfileForm.password.trim().length > 0 && !lockSuperAdminPassword && (
                            <div className="mt-2">
                                <label className="text-xs uppercase text-slate-500 font-bold">Sua senha atual</label>
                                <input
                                    type="password"
                            ...
                            {saveTab === 'racks' && (
                                <div className="space-y-2">
                                    {(selectedUserSave.placedRacks || []).map((rack, idx) => {
                                        ...
                                        const rawBat = rack.batteryId;
                                        const battCatId = resolveRackBatteryItemIdWithWorkshopFallback(
                                            rawBat,
                                            gameUpgrades,
                                            selectedUserSave.storedBatteries || [],
                                            selectedUserSave.workshopSlots
                                        );
                                        const battUp = battCatId ? gameUpgrades.find((u) => u.id === battCatId) : undefined;
                                        const batLooksLikeCatalogKey = !!(rawBat && gameUpgrades.some((u) => u.id === rawBat));
                                        return (
                                                            ...
                                                            <div>
                                                                ...
                                                                {!rawBat ? (
                                                                    ...
                                                                    <span className="text-slate-200">
                                                                        {battUp.name}
                                                                        {!batLooksLikeCatalogKey ? (
                                                                            <span className="block font-mono text-[8px] text-slate-600 truncate max-w-[11rem]" title={rawBat}>
                                                                                inst. {rawBat.length > 24 ? `${rawBat.slice(0, 10)}…${rawBat.slice(-6)}` : rawBat}
                                                                            </span>
                                                                        ) : null}
                                                                    </span>
                                                                ) : (
                                                                    <span className="text-amber-500/90">
                                                                        sem catálogo
                                                                        <span className="block font-mono text-[8px] text-slate-600 truncate max-w-[11rem]" title={rawBat}>
                                                                            {rawBat.length > 24 ? `${rawBat.slice(0, 10)}…${rawBat.slice(-6)}` : rawBat}
                                                                        </span>
                                                                    </span>
                                                                )}
                                                            </div>
