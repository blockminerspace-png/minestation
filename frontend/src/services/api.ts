...
export type PublicBootstrapPayload = {
  ...
  /** URL público canónico (FRONTEND_URL / etc. no servidor); referral usa isto em vez do host da página quando definido. */
  appOrigin: string | null;
};

export type PublicBootstrapLitePayload = Pick<
  PublicBootstrapPayload,
  'accessLevels' | 'miningCoins' | 'economySettings' | 'lootBoxes' | 'web3Settings' | 'appOrigin'
>;
  ...
  return {
    ...
    gameNavLabels:
      d.gameNavLabels != null && typeof d.gameNavLabels === 'object' && !Array.isArray(d.gameNavLabels)
        ...
        : {},
    appOrigin:
      typeof d.appOrigin === 'string' && d.appOrigin.trim()
        ? d.appOrigin.trim().replace(/\/+$/, '')
        : null
  };
}
    ...
    if (!res.ok) {
      if (res.status === 413) {
        try {
          ...
          if (j?.error && typeof j.error === 'string') return { ok: false, error: j.error };
        } catch {
          /* fall through */
        }
        return {
          ok: false,
          error:
            'Ficheiro demasiado grande (limite do servidor). Comprime a imagem ou GIF (~20 MB) e tenta de novo; em produção confirma nginx client_max_body_size.'
        };
      }
      const text = await res.text();
...
export async function updateUser(
  user: User & { newReferralFor?: string; currentPassword?: string }
): Promise<{ ok: boolean; error?: string; code?: string; accounts?: any[] }> {
  try {
...
export type ServerRoomBulkBatteriesPreviewSuccess = {
  ok: true;
  smartFillOk: boolean;
  smartFillMessage?: string;
...
/** Pré-visualização alinhada com POST `/api/server-room/bulk-batteries` (sem escrita). */
export async function getServerRoomBulkBatteriesPreview(params: {
  roomId: string;
  rigSort?: 'slot_asc' | 'hashrate_desc';
  smartFill?: boolean;
  batteryUpgradeId?: string;
  signal?: AbortSignal;
}): Promise<ServerRoomBulkBatteriesPreviewSuccess | { ok: false; error: string }> {
  try {
    const q = new URLSearchParams();
    q.set('roomId', params.roomId);
    if (params.rigSort) q.set('rigSort', params.rigSort);
    if (params.smartFill) q.set('smartFill', '1');
    if (params.batteryUpgradeId != null && params.batteryUpgradeId !== '') {
      q.set('batteryUpgradeId', params.batteryUpgradeId);
    }
    const res = await apiFetch(`${base}/server-room/bulk-batteries-preview?${q.toString()}`, {
      method: 'GET',
    ...
    return data as unknown as ServerRoomBulkBatteriesPreviewSuccess;
  } catch (e: unknown) {
    const name = e && typeof e === 'object' && 'name' in e ? String((e as { name?: string }).name) : '';
    if (name === 'AbortError') return { ok: false, error: '__abort__' };
    return { ok: false, error: 'Network error' };
  }
}
...
> {
  try {
    ...
    if (!res.ok) {
      const err = typeof data.error === 'string' ? data.error : `HTTP ${res.status}`;
      return { ok: false, error: err, forceReload: !!data.forceReload };
    }
...
export async function postServerRoomRoomCoins(
  ...
  | { ok: true; serverUpdatedAt: number; placedRacks: PlacedRack[]; forceReload?: boolean }
  | { ok: false; error: string; forceReload?: boolean }
> {
  try {
  ...
  try {
    ...
    if (!res.ok) {
      try {
        ...
        const errMsg = typeof errBody.error === 'string' && errBody.error.trim() ? errBody.error.trim() : `HTTP ${res.status}`;
        return {
          ok: false,
          error: errMsg,
          ...(errBody.forceReload ? { forceReload: true as const } : {})
        };
      } catch {
...
export type AdminMaintenanceState = { active: boolean; message: string | null };

export async function getAdminMaintenance(): Promise<AdminMaintenanceState | null> {
  try {
    const res = await apiFetch(`${base}/admin/maintenance`);
    if (!res.ok) return null;
    return await res.json();
  } catch {
    return null;
...
export async function updateAdminMaintenance(payload: {
  active: boolean;
  message?: string;
}): Promise<
  | { ok: true; active: boolean; message: string | null; invalidated?: { sessions: number; jwtRefresh: number } }
...
> {
  try {
    ...
    const data = (await res.json().catch(() => ({}))) as {
      ...
      active?: boolean;
      message?: string | null;
      invalidated?: { sessions: number; jwtRefresh: number };
      error?: string;
    };
    if (!res.ok) {
      return { ok: false, error: data.error || `Erro HTTP ${res.status}` };
