...
export type UserPutCoreTxInput = {
  ...
  /** Se `false`, não altera `polygon_wallet` (evita apagar carteira quando o cliente não envia o campo). */
  updatePolygonWallet: boolean;
...
export async function executeUserPutCoreTransaction(
  ...
  const userUpdateBase = {
    ...
    ...(updatePolygonWallet ? { polygon_wallet: polygonForUpdate } : {})
  };
