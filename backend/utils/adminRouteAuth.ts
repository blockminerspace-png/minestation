...
export function resolveAdminRouteRequirement(method: string, rawPath: string): AdminRouteRequirement {
  ...
  if (p === '/api/admin/maintenance') return { kind: 'super' };
  ...
  if (p === '/api/admin/battery-possession-audit') return { kind: 'tab', tab: 'users' };
  if (p === '/api/admin/battery-possession-fix') return { kind: 'super' };
