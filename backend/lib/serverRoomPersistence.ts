...
export async function persistStockStoredBatteriesPlacedRacks(
  ...
  if (storedBatteries) {
    ...
    const keepIds = await storedBatteryKeepIdsForSave(
      client,
      uid,
      incomingIds,
      changes.placedRacks,
    ...
    if (keepIds.length > 0) {
      await client.query('DELETE FROM stored_batteries WHERE user_id = $1 AND NOT (id = ANY($2::text[]))', [uid, keepIds]);
    } else {
  ...
  if (placedRacks) {
    ...
    // Guarda com SQL na mesma conexão `pg` (transação da sala); o save-game usa Prisma no `tx`.
    assertPlacedRacksSlotPayloadAgainstPrevDb(
      prevRacksRes.rows as Array<{ id: string; item_id?: unknown }>,
      prevSlotsRes.rows as Array<{ rack_id: string; slot_index: unknown; machine_item_id?: unknown }>,
      prevMultRes.rows as Array<{ rack_id: string; slot_index: unknown; multiplier_item_id?: unknown }>,
      placedRacks,
      { adminOverride: false, userId: uid }
    );
