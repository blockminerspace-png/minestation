/**
 * Regras autoritativas de bateria em massa na sala (espelho histórico do modelo no frontend; UI usa pré-visualização/API).
...
function unloadRackBatteryToInventory(
  ...
  if (mountedRow) {
    ...
    if (isFull) {
      ...
      mountedRow.currentCharge = typeof rack.currentCharge === 'number' ? rack.currentCharge : mountedRow.currentCharge;
    }
  }
}
...
export function applyBulkRoomBatteryChange(
  ...
  const totalAvail = totalBatteryInstances(batteryUpgradeId, prev.stock, prev.storedBatteries || [], prev.placedRacks);
  if (totalAvail <= 0) {
