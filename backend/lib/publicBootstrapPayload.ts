...
export type PublicBootstrapPayload = {
  ...
  /**
   * URL público canónico do SPA (sem barra final), alinhado a `FRONTEND_URL` / `PUBLIC_URL` / `SITE_URL`
   * na mesma ordem que o `primary` em `server.ts` (CORS). Usado no cliente para links de referral.
   */
...
/** Mesma prioridade que o URL público em `server.ts` (`buildCorsOriginSet` / log `[CORS]`). */
export function resolveCanonicalPublicAppOrigin(): string | null {
  const raw =
...
export async function getPublicBootstrapPayload(userId: number | undefined, lite: boolean): Promise<unknown> {
  const appOrigin = resolveCanonicalPublicAppOrigin();
  if (lite) {
