/**
 * Alinha `stored_batteries.status/location/rack_id/...` com `placed_racks` e oficina
 * após persistência de sala (`persistStockStoredBatteriesPlacedRacks`).
  ...
  await client.query(
    ...
    UPDATE stored_batteries sb
       SET status = 'CHARGING',
           ...
           room_id = NULL,
           version = CASE
             WHEN sb.status IS DISTINCT FROM 'CHARGING'::text
               OR sb.location IS DISTINCT FROM 'WORKSHOP_CHARGER'::text
