...
#
# Uso (cwd = pasta backend, ou qualquer cwd com caminhos absolutos em DUMP_PATH/COMPOSE_FILE):
#   bash scripts/import_pg_dump.sh
#   DUMP_PATH=/outro.sql bash scripts/import_pg_dump.sh
#   SKIP_LOCAL_DOWN=1 bash scripts/import_pg_dump.sh   # não derruba o compose local
#   COMPOSE_FILE=docker-compose.yml bash scripts/import_pg_dump.sh   # explícito (já é o default)
