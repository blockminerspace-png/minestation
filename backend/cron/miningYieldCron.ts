...
type RackRow = {
  ...
  id: string;
  user_id: number;
  /** `placed_racks.battery_id` pode ser id de upgrade (stock) ou UUID de `stored_batteries`. */
  battery_upgrade_id: string;
...
export async function updateMiningYields(pool: Pool): Promise<void> {
  ...
  try {
    const activeRes = await client.query(`
      SELECT pr.selected_coin_id, pr.id, pr.user_id, pr.current_charge, u.username,
        COALESCE(sb.item_id, pr.battery_id) AS battery_upgrade_id
      FROM placed_racks pr
      JOIN users u ON pr.user_id = u.id
      JOIN mining_coins mc ON pr.selected_coin_id = mc.id
      LEFT JOIN stored_batteries sb ON sb.user_id = pr.user_id AND sb.id = pr.battery_id
      WHERE pr.is_on = 1
      AND mc.is_active = 1
    ...
    for (let i = 0; i < racks.length; i += BATCH_SIZE) {
      ...
      for (const rack of batch) {
        ...
        const batt = upsMap.get(String(rack.battery_upgrade_id));
