...
import { resolveBatteryPolymorphicToCatalogItemId } from '../lib/batteryPolymorphicResolve.js';
import {
  batteryIdLooksLikePhysicalInstanceUuid,
  resolveRackBatteryIdsToCatalogItemIds
...
export async function computeProgressForUser(
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      const rackBatteryIds = [
        ...new Set(
          rows
            .map((r) => (r.battery_id != null && String(r.battery_id).trim() !== '' ? String(r.battery_id).trim() : ''))
            .filter((x) => x.length > 0)
        )
      ];
      const rackBatteryToCatalog =
        rackBatteryIds.length > 0
          ? (await resolveRackBatteryIdsToCatalogItemIds(client, userId, rackBatteryIds, {})).rackBatteryToCatalog
      ...
      for (const r of rows) {
        ...
        const rawBat = r.battery_id ? String(r.battery_id).trim() : '';
        const catalogBat =
          rawBat &&
          (rackBatteryToCatalog.get(rawBat) ||
            (!batteryIdLooksLikePhysicalInstanceUuid(rawBat) ? rawBat : ''));
        const battDef = catalogBat ? upgradesMap.get(catalogBat) : null;
    ...
    for (const ws of workshopRes.rows as Array<Record<string, unknown>>) {
      ...
      for (const bSlot of batterySlots) {
        ...
        const slotHintRaw = getSlotVal(slotItemIds, bSlot.id);
        const slotHint = slotHintRaw != null && String(slotHintRaw).trim() !== '' ? String(slotHintRaw).trim() : null;
        const catalogItemId = await resolveBatteryPolymorphicToCatalogItemId(
          client,
          userId,
          String(batteryInstanceId),
          slotHint
        );
        const batteryDef = catalogItemId ? upgradesMap.get(catalogItemId) : undefined;
