 ...
 */
export function createResolveAuthMiddleware({ db, parseCookies, allowLegacySession = true }: ResolveAuthDeps) {
  return async function resolveAuth(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if (sid) {
      try {
        ...
        const s = sRes.rows[0] as { user_id: number; expires_at: number } | undefined;
...
export async function issueJwtAuthCookies(db: Pool, res: Response, userId: number, req: Request): Promise<void> {
...
export async function revokeJwtRefreshForUser(db: Pool, userId: number): Promise<void> {
  await revokeAllRefreshForUser(db, userId);
  await writeJwtRefreshSnapshot(db);
}
