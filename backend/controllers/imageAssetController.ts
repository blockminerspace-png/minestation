...
/** Banners laterais/GIF podem exceder 5 MB; alinhado ao `client_max_body_size` do nginx (~32m). */
const ADMIN_AD_UPLOAD_MAX_BYTES = 20 * 1024 * 1024;
...
function createAdminAdMulter(uploadsDir: string): ReturnType<typeof multer> {
  ...
  return multer({
    storage: adStorage,
    limits: { fileSize: ADMIN_AD_UPLOAD_MAX_BYTES },
      ...
      if (err) {
        ...
        if (code === 'LIMIT_FILE_SIZE') {
          return res.status(413).json({
            error: `Ficheiro demasiado grande (máximo ${Math.round(ADMIN_AD_UPLOAD_MAX_BYTES / (1024 * 1024))} MB). Comprime a imagem ou GIF.`
          });
        }
