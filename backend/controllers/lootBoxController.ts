...
export function registerLootBoxPlayerRoutes(app: Express, deps: LootBoxPlayerDeps): void {
  ...
  app.post('/api/loot-boxes/open', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, err)) {
        console.warn('[BoxOpen] db_conflict', err instanceof Error ? err.message : String(err));
        return;
      }
  ...
  app.post('/api/loot-boxes/buy', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, err)) {
        console.warn('[BoxBuy] db_conflict', err instanceof Error ? err.message : String(err));
        return;
      }
  ...
  app.post('/api/loot-boxes/discard', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, err)) {
        console.warn('[BoxDiscard] db_conflict', err instanceof Error ? err.message : String(err));
        return;
      }
