...
export function registerRoletaPlayerRoutes(app: Express, deps: RoletaPlayerDeps): void {
  ...
  app.get('/api/roleta/pending-code', authenticateToken, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[Roleta pending-code] db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
  ...
  app.post('/api/wheel/roll', authenticateToken, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[Wheel Roll] db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
  ...
  app.get('/api/wheel/paid-pending', authenticateToken, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[Wheel paid-pending] db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
  ...
  app.post('/api/wheel/paid-roll', authenticateToken, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[Wheel paid-roll] db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
  ...
  app.post('/api/wheel/paid-claim', authenticateToken, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[Wheel paid-claim] db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
  ...
  app.post('/api/roleta/claim', authenticateToken, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[Roleta Claim] db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
