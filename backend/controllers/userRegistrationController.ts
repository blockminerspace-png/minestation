  ...
  app.put('/api/user', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (req.userId) {
        ...
        if (Object.prototype.hasOwnProperty.call(u, 'polygonWallet')) {
      ...
      if (hasPassword && isAuthenticatedRequest) {
        const changingOwnAccount = Number(uid) === Number(req.userId);
        if (changingOwnAccount) {
          ...
          if (stored.length > 0) {
            const curRaw = typeof u.currentPassword === 'string' ? u.currentPassword : '';
            if (!curRaw.trim()) {
              return res.status(400).json({
                error: 'Indique a palavra-passe atual para definir uma nova palavra-passe.'
              });
            }
            let ok = false;
            if (stored.startsWith('$2a$') || stored.startsWith('$2b$')) {
              try {
                ok = await bcrypt.compare(curRaw, stored);
              } catch {
                ok = false;
              }
            } else {
            ...
            if (!ok) {
              return res.status(403).json({ error: 'Palavra-passe atual incorreta.' });
            }
          }
        }
      }
      ...
      const updatePolygonWallet =
        !isAuthenticatedRequest || Object.prototype.hasOwnProperty.call(u, 'polygonWallet');
