...
import { trySendTransactionalDbConflictResponse } from '../utils/transactionalDbConflictResponse.js';
...
export function registerP2pMarketRoutes(app: Express, deps: P2pMarketDeps): void {
  ...
  app.get('/api/market/listings', async (req: Request, res: Response) => {
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[P2P] listings db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
  ...
  app.get('/api/market/history', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[P2P] history db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
  ...
  app.post('/api/market/sell', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[P2P] sell db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
  ...
  app.post('/api/market/cancel', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[P2P] cancel db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
  ...
  app.post('/api/market/reserve', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e, { mergeBody: { ok: false } })) {
        console.warn('[P2P] reserve db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
  ...
  app.post('/api/market/cancel-reserve', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[P2P] cancel-reserve db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
      sendInternalErrorSafeMessageOrPrisma(res, req.originalUrl || 'p2p', e, 'Erro.');
    }
  ...
  app.post('/api/market/buy', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[P2P] buy db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
      console.error('[P2P] buy error', e);
      sendInternalErrorSafeMessageOrPrisma(res, req.originalUrl || 'p2p', e, 'Erro na compra.');
    }
  });
  ...
  app.post('/api/market/claim', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[P2P] claim db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
      sendInternalErrorSafeMessageOrPrisma(res, req.originalUrl || 'p2p', e, 'Erro.');
    }
  ...
  app.get('/api/market/custody', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[P2P] custody db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
      sendInternalErrorSafeMessageOrPrisma(res, req.originalUrl || 'p2p', e, 'Erro.');
    }
  ...
  app.post('/api/market/claim-item', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (trySendTransactionalDbConflictResponse(res, e)) {
        console.warn('[P2P] claim-item db_conflict', e instanceof Error ? e.message : String(e));
        return;
      }
      sendInternalErrorSafeMessageOrPrisma(res, req.originalUrl || 'p2p', e, 'Erro.');
    }
