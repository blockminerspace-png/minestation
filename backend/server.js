...
app.post('/api/upgrades/buy', async (req, res) => {
    ...
    try {
       ...
       FROM upgrades WHERE id = ANY($1::text[]) ORDER BY id FOR UPDATE`, [upgradeIds]);
        ...
        const now = Date.now();
        const deductRes = await client.query(`UPDATE game_states SET usdc = $1, last_updated_at = $2, server_updated_at = $2
       WHERE user_id = $3 AND usdc >= $4`, [newUsdc, now, uid, totalCost]);
        ...
        for (const item of itemsToBuy) {
         ...
         ON CONFLICT (user_id, item_id) DO UPDATE SET qty = stock.qty + EXCLUDED.qty`, [uid, item.id, item.qty]);
        }
        await client.query('COMMIT');
        const unameRes = await db.query('SELECT username FROM users WHERE id = $1', [uid]);
        const username = unameRes.rows[0]?.username || '';
        console.log('[HardwareBuy] ts=%s userId=%s username=%s totalUsdc=%s newUsdc=%s lines=%s', new Date().toISOString(), uid, username, totalCost.toFixed(6), newUsdc.toFixed(6), JSON.stringify(itemsToBuy.map((i) => ({ id: i.id, qty: i.qty, name: i.name }))));
...
app.get('/api/load-game', async (req, res) => {
    ...
    try {
        ...
        const storedBatteries = batRows.map((r) => ({
            ...
            workshopSlotIndex: r.workshop_slot_index != null
                ? Number(r.workshop_slot_index)
                : null,
            workshopComponentSlotId: r.workshop_component_slot_id != null
                ? String(r.workshop_component_slot_id)
                : null
        }));
        const racks = [];
        ...
        if (rackIds.length > 0) {
            ...
            for (const r of rackRows) {
                racks.push({
                    ...
                    batteryPowerCapacityWh: r.battery_power_capacity_wh != null
                        ? Number(r.battery_power_capacity_wh)
                        : null,
    ...
    try {
        ...
        ON CONFLICT (user_id) DO UPDATE SET
          ...
          ON CONFLICT (id) DO UPDATE SET
            ...
            workshop_component_slot_id = stored_batteries.workshop_component_slot_id`, [uid, bIds, bItemIds, bCharges, bPowers, bNames, bImgs]);
                }
            }
            if (changes.placedRacks) {
         ...
         ORDER BY s.rack_id, s.slot_index`, [uid]);
                    ...
                    for (const r of changes.placedRacks) {
                        ...
                        const prevBatt = prow
                            ? {
                                ...
                                battery_power_capacity_wh: prow.battery_power_capacity_wh != null ? Number(prow.battery_power_capacity_wh) : null,
          ...
          ON CONFLICT (id) DO UPDATE SET
            ...
            if (changes.workshopSlots) {
                ...
                const linkageSlots = [null, null, null, null, null, null];
...
app.post('/api/game/save-workshop', async (req, res) => {
    ...
    const wsChanges = { lastLoadTime, workshopSlots };
    if (stock !== undefined)
        wsChanges.stock = stock;
    if (storedBatteries !== undefined)
        wsChanges.storedBatteries = storedBatteries;
    req.body = {
        changes: wsChanges,
        adminOverride: false
    };
...
app.post('/api/exchange/sell', async (req, res) => {
    ...
    try {
       ...
       FROM mining_coins WHERE id = $1`, [cid]);
        ...
        const balRes = await client.query('SELECT amount FROM coin_balances WHERE user_id = $1 AND coin_id = $2 FOR UPDATE', [uid, cid]);
        ...
        const updCoin = await client.query('UPDATE coin_balances SET amount = amount - $1 WHERE user_id = $2 AND coin_id = $3 AND amount >= $1 RETURNING amount', [sellAmount, uid, cid]);
        ...
        console.log('[ExchangeSell] userId=%s coinId=%s coinName=%s pct=%s soldAmount=%s grossUsdc=%s feeUsdc=%s netUsdc=%s', uid, cid, coinDef.name, String(pct), sellAmount, grossUsdc.toFixed(8), feeAmount.toFixed(8), netUsdc.toFixed(8));
