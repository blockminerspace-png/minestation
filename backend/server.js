...
import { runReferralCommissionOnTx, creditDepositReferralCommissionPg, newAdminUsdcGiftReferralIdempotencyKey } from './dist/models/referralCommissionModel.js';
...
function ensureAdminSettingsLabelsInPermissions(permissions) {
    if (!Array.isArray(permissions))
        return permissions;
    if (permissions.includes('settings:labels'))
        return permissions;
...
const ensureUserLevels = async () => {
    try {
        ...
        console.error('Erro ao restaurar níveis:', e);
    }
};
...
const parseCookies = (req) => {
    ...
    return header.split(';').map(v => v.trim()).filter(Boolean).reduce((acc, cur) => { const i = cur.indexOf('='); if (i > 0)
        acc[cur.slice(0, i)] = cur.slice(i + 1); return acc; }, {});
};
try {
    getJwtAuthConfig();
}
...
function buildCorsOriginSet() {
    ...
    const primaryPublic = process.env.FRONTEND_URL ||
...
{
    const primary = normalizeCorsOrigin(process.env.FRONTEND_URL ||
        process.env.PUBLIC_URL ||
        process.env.SITE_URL ||
        process.env.VITE_APP_URL ||
        '') || '(nenhuma)';
...
const enforceHttpsRedirect = process.env.NODE_ENV === 'production' && String(process.env.ENFORCE_HTTPS ?? '1') !== '0';
...
function sanitizePlayerActivityMeta(raw) {
    ...
    if (raw.smartFill === true)
        out.smartFill = true;
    if (raw.smartFill === false)
        out.smartFill = false;
    if (raw.rigSort === 'slot_asc' || raw.rigSort === 'hashrate_desc')
        out.rigSort = raw.rigSort;
    for (const k of ['appliedRigs', 'compatibleRigs']) {
        const n = raw[k];
        if (typeof n === 'number' && Number.isFinite(n) && n >= 0 && n <= 100000)
            out[k] = Math.floor(n);
    }
    if (raw.ok === true || raw.ok === false)
        out.ok = raw.ok;
    return out;
}
function sanitizePlayerActivityClientHints(raw, depth = 0) {
    ...
    for (const [k, v] of Object.entries(raw)) {
        if (count++ > 48)
            break;
        if (typeof k !== 'string' || k.length > 64 || !/^[a-zA-Z0-9_]+$/.test(k))
            continue;
        if (typeof v === 'string')
            out[k] = v.slice(0, 600);
...
app.post('/api/player-activity-log', async (req, res) => {
    ...
    if (action === 'room_battery_smart') {
        if (meta.smartFill !== true)
            return res.status(400).json({ error: 'Dados inválidos' });
        if (meta.batteryUpgradeId)
            return res.status(400).json({ error: 'Dados inválidos' });
    }
            ...
            for (const item of items) {
                await tx.wheel_prizes.create({
                    data: {
                        ...
                        tier: item.tier != null && String(item.tier).trim() !== ''
...
app.post('/api/admin/wheel/runtime-config', isAdmin, async (req, res) => {
    ...
    try {
        ...
        const dailyLimit = b.dailyLimit === null || b.dailyLimit === undefined || b.dailyLimit === ''
            ? null
...
app.get('/api/admin/economy-stats', isAdmin, async (req, res) => {
    ...
    `);
        ...
        for (const rack of racksRes.rows) {
            ...
            if (!coinsMap.has(cid))
                continue;
            // Sistema de carga removido em 20260516180000: rig com bateria atribuída produz sempre.
            // Base Prod
            let base = 0;
            ...
            if (!activeMinersSets[cid])
                activeMinersSets[cid] = new Set();
...
const assertPublicSignupEmailAllowed = (normalizedEmail) => {
    ...
    if (SIGNUP_EMAIL_ALLOWLIST.has(domain))
        return { ok: true };
    return {
        ok: false,
...
app.post('/api/admin-upgrades/purchase', async (req, res) => {
    ...
    const clientPackageVersion = clientVRaw == null || clientVRaw === ''
        ? null
        : typeof clientVRaw === 'number'
            ? clientVRaw
            : parseInt(String(clientVRaw), 10);
    const clientPackageVersionNorm = clientPackageVersion != null && Number.isFinite(clientPackageVersion) ? clientPackageVersion : null;
    try {
...
app.get('/api/applixir-callback', async (req, res) => {
    ...
    try {
        ...
        // Sistema de carregamento (oficina + reward-ad) descontinuado: o callback
        // continua a aceitar a chamada para compatibilidade com a Applixir, mas não
        // recarrega nada — baterias são infinitas (UUID em stored_batteries).
        res.send('OK');
    }
    catch (e) {
...
// Endpoints removidos em 20260516180000_battery_uuids_and_purge_charging:
//   POST /api/workshop/recharge
//   POST /api/reward-ad
//   POST /api/daily-boost
//   POST /api/admin/reset-daily-boost
// Baterias são UUID infinitas; nada para recarregar.
...
const depositBlockscoutEthRpcUrl = (net) => {
    ...
    if (n === 'bnb' || n === 'bsc')
        return 'https://bsc.blockscout.com/api/eth-rpc';
    if (n === 'base')
...
function parseProxyEthReceiptResponse(j) {
    if (!j || typeof j !== 'object')
        return null;
    if (String(j.status || '') === '0' && j.message && !j.result)
        return null;
    let result = j.result;
...
async function fetchDepositReceiptUnified(net, rpcUrl, txHash) {
    ...
    if (fromBlockscout && fromBlockscout.result)
    ...
    if (fromExplorer && fromExplorer.result)
        return fromExplorer;
...
function isDepositNetworkDisabledInSettings(settings, network) {
    ...
    if (n === 'polygon' || n === 'matic')
        return settingsFlagEnabled(settings.web3_deposit_polygon_disabled);
    if (n === 'bnb' || n === 'bsc')
        return settingsFlagEnabled(settings.web3_deposit_bnb_disabled);
    if (n === 'base')
        return settingsFlagEnabled(settings.web3_deposit_base_disabled);
    return true;
}
...
async function queueDepositPending(txHash, payload) {
    ...
    await db.query(`INSERT INTO app_cache (key, value, updated_at) VALUES ($1, $2::jsonb, NOW())
     ON CONFLICT (key) DO UPDATE SET value = $2::jsonb, updated_at = NOW()`, [key, JSON.stringify(payload)]);
}
...
async function tryCreditDepositFromReceipt(uid, txHash, net, settings, receipt) {
    ...
    const contractCandidates = netLower === 'polygon' || netLower === 'matic'
    ...
    const walletRow = await db.query("SELECT lower(trim(COALESCE(polygon_wallet::text, ''))) AS w FROM users WHERE id = $1", [uid]);
    ...
    try {
        const decRes = await fetchWithTimeout(rpcUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                jsonrpc: '2.0',
    ...
    if (isNaN(amountUsdc))
        return { ok: false, error: 'Erro ao calcular o valor do depósito' };
    ...
    if (amountUsdc <= 0)
        return { ok: false, error: 'Valor zero na transação' };
    const client = await db.connect();
    try {
        await client.query('BEGIN');
        ...
        const now = Date.now();
        await client.query(`UPDATE game_states SET usdc = COALESCE(usdc, 0) + $1,
        total_usdc_deposited = COALESCE(total_usdc_deposited, 0) + $1,
        server_updated_at = $2, last_updated_at = $2 WHERE user_id = $3`, [amountUsdc, now, uid]);
        ...
        console.log('[DepositCredit] userId=%s network=%s amountUsdc=%s tx=%s newUsdc=%s', uid, net, Number(amountUsdc).toFixed(8), String(txHash).slice(0, 18) + '…', newBalRes.rows[0].usdc);
        if (amountUsdc > 0) {
...
async function sweepPendingDepositsOnce() {
    try {
        ...
        for (const row of rows.rows) {
            ...
            if (r.ok) {
                if (r.amount > 0) {
                    console.log('[DepositSweep] credited userId=%s network=%s amountUsdc=%s tx=%s', userId, network, Number(r.amount).toFixed(8), String(txHash).slice(0, 18) + '…');
                }
            }
            else if (!r.pending) {
...
app.post('/api/deposit/verify', async (req, res) => {
    ...
    const uid = req.userId;
    if (!uid)
        return res.status(401).json({ error: 'Sessão necessária para verificar depósito.' });
...
app.post('/api/nfts/receive', isAdmin, async (req, res) => {
    ...
    try {
      ...
      ON CONFLICT (contract_address, token_id) DO UPDATE SET owner_address = $3`, [contract, tokenId, toAddress]);
        res.json({ ok: true });
    }
    catch (e) {
        sendInternalErrorOrPrisma(res, req.originalUrl || 'api', e);
...
app.post('/api/nfts/send', async (req, res) => {
    ...
    try {
        ...
        if (!contract || !tokenId || !fromAddress || !toAddress)
            return res.status(400).json({ error: 'Missing fields' });
        if (!allowed.some(c => c.toLowerCase() === contract.toLowerCase()))
            return res.status(403).json({ error: 'Contract not allowed' });
...
app.get('/api/access-levels', async (req, res) => {
    try {
        ...
        const levels = rows.map((r) => ({
            ...
            allowedPages: r.allowed_pages ? (() => { try {
                return JSON.parse(r.allowed_pages);
            }
            catch {
                return [];
            } })() : []
...
async function sanitizePlacedRacksNftAutoRoom(client, uid, changes, saveActivityLogs) {
    ...
    for (const r of racks) {
        ...
        if (r.wiringId) {
            ...
            if (w)
                stock[w] = Math.floor((Number(stock[w]) || 0) + 1);
        }
        for (const slot of r.slots || []) {
            ...
            if (sid)
                stock[sid] = Math.floor((Number(stock[sid]) || 0) + 1);
        }
        for (const slot of r.multiplierSlots || []) {
            ...
            if (mid)
                stock[mid] = Math.floor((Number(stock[mid]) || 0) + 1);
        }
        try {
...
app.post('/api/server-room/room-coins', async (req, res) => {
    ...
    if (rawCoin != null && String(rawCoin).trim() !== '') {
        ...
        if (!RACK_ID_RE.test(selectedCoinId))
    ...
    try {
        ...
        await client.query(`UPDATE placed_racks SET
        selected_coin_id = $1::text,
        is_on = CASE WHEN $1::text IS NULL THEN 0 ELSE is_on END
      WHERE user_id = $2 AND ${roomSqlExpr} = $3::text`, [selectedCoinId, uid, roomNorm]);
        const finalServerUpdatedAt = Date.now();
        await client.query(`UPDATE game_states SET last_updated_at = $1, server_updated_at = $2 WHERE user_id = $3`, [finalServerUpdatedAt, finalServerUpdatedAt, uid]);
        await client.query('COMMIT');
...
app.post('/api/rig-rooms/purchase-slot', async (req, res) => {
    ...
    const roomId = typeof roomIdRaw === 'string' && RIG_ROOM_ID_RE.test(roomIdRaw.trim()) ? roomIdRaw.trim() : null;
    if (!roomId)
    ...
    if (!Number.isFinite(quantity) || quantity < 1)
        quantity = 1;
    if (quantity > 50)
        return res.status(400).json({ ok: false, error: 'Máximo de 50 slots por compra.' });
    const uid = req.userId;
    const client = await db.connect();
    try {
        await client.query('BEGIN');
        ...
        if (userRoom) {
            await client.query('UPDATE user_rig_rooms SET unlocked_slots = unlocked_slots + $1 WHERE user_id = $2 AND room_id = $3', [n, uid, roomId]);
        }
        else {
            await client.query('INSERT INTO user_rig_rooms (user_id, room_id, purchased_at, unlocked_slots) VALUES ($1, $2, $3, $4)', [uid, roomId, Date.now(), n]);
        }
        await client.query('COMMIT');
...
app.post('/api/news', isAdmin, async (req, res) => {
    ...
    try {
        await db.query('INSERT INTO system_news (id,text,link,active,duration,author_name,created_at,ad_type,image_url) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (id) DO UPDATE SET text = $2, link = $3, duration = $5, author_name = $6, ad_type = $8, image_url = $9', [id, text, link ?? null, 1, duration ?? null, authorName ?? 'Admin', Date.now(), adType ?? 'horizontal', imageUrl ?? null]);
        res.json({ ok: true });
    }
    catch (e) {
        sendInternalErrorOrPrisma(res, req.originalUrl || 'api', e);
...
app.post('/api/admin/transparency', isAdmin, async (req, res) => {
    try {
        ...
        if (t.length > 300)
            return res.status(400).json({ error: 'Título longo demais' });
        const b = body != null ? String(body).trim() : '';
        if (b.length > 8000)
            return res.status(400).json({ error: 'Descrição longa demais' });
        let amt = null;
        if (amountUsdc !== undefined && amountUsdc !== null && String(amountUsdc).trim() !== '') {
            const n = Number(amountUsdc);
            if (!Number.isFinite(n))
                return res.status(400).json({ error: 'Valor USDC inválido' });
            amt = n;
        }
...
app.post('/api/admin/display-labels', isAdmin, async (req, res) => {
    ...
    try {
        ...
        for (const [key, rawVal] of Object.entries(raw)) {
            if (!UI_DISPLAY_LABEL_KEY_SET.has(key))
                continue;
            ...
            if (!val) {
                ...
                await client.query(`INSERT INTO ui_display_labels (key, value, updated_at) VALUES ($1, $2, $3)
           ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = EXCLUDED.updated_at`, [key, val, now]);
            }
        }
        await client.query('COMMIT');
...
app.post('/api/player-news/submit', async (req, res) => {
    ...
    try {
        ...
        if (bal < fee)
            return res.status(400).json({ error: 'Insufficient USDC', missing: fee - bal });
        await client.query('BEGIN');
        if (fee > 0)
...
app.post('/api/player-news/approve', isAdmin, async (req, res) => {
    ...
    try {
        ...
        const u = uRes.rows[0];
        await client.query('INSERT INTO system_news (id,text,link,active,duration,author_name,created_at) VALUES ($1,$2,$3,$4,$5,$6,$7)', [crypto.randomUUID(), row.text, row.link ?? null, 1, 60, u?.username || '', Date.now()]);
        await client.query('COMMIT');
        res.json({ ok: true });
    }
    catch (e) {
...
app.post('/api/season-passes', isAdmin, async (req, res) => {
    ...
    try {
        ...
        for (const p of passes) {
            ...
            const updRes = await client.query('UPDATE season_passes SET season_id = $1, name = $2, description = $3, price_usdc = $4, emblem_url = $5, is_active = $6 WHERE id = $7', [p.seasonId, p.name, p.description, p.priceUsdc, p.emblemUrl ?? null, p.isActive ? 1 : 0, p.id]);
            if (updRes.rowCount === 0) {
                await client.query('INSERT INTO season_passes (id,season_id,name,description,price_usdc,emblem_url,is_active) VALUES ($1,$2,$3,$4,$5,$6,$7)', [p.id, p.seasonId, p.name, p.description, p.priceUsdc, p.emblemUrl ?? null, p.isActive ? 1 : 0]);
            }
            ...
            if (p.rewards && Array.isArray(p.rewards)) {
                for (const r of p.rewards) {
                    await client.query('INSERT INTO season_pass_rewards (pass_id, type, item_id, coin_id, qty) VALUES ($1, $2, $3, $4, $5)', [p.id, r.type, r.itemId || null, r.coinId || null, r.qty || 0]);
                }
            }
        }
    ...
    try {
      ...
      LIMIT $2::int OFFSET $3::int`;
        ...
        res.json({
            ...
            noMiningTotal,
            miningNoWalletTotal,
            note: 'Data de referência: game_states.start_time (criação do save no registo). «Minerar»: pelo menos uma rig com is_on=1. Carteira: users.polygon_wallet. Paginação: noMiningPage / miningNoWalletPage + limit (50–500).',
...
app.post('/api/admin/referral-models', isAdmin, async (req, res) => {
    ...
    try {
        ...
        WHERE id=$8`, [name, description, sender_reward_usdc || 0, receiver_reward_usdc || 0, sender_loot_box_id || null, receiver_loot_box_id || null, is_active ? 1 : 0, id]);
            res.json({ ok: true });
        }
        else {
            await db.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, $7)`, [name, description, sender_reward_usdc || 0, receiver_reward_usdc || 0, sender_loot_box_id || null, receiver_loot_box_id || null, is_active ? 1 : 0]);
            res.json({ ok: true });
        }
    }
    catch (e) {
...
async function deleteUserByEmail(email, client) {
    ...
    try {
        ...
        if (userRes.rowCount > 1) {
            const exactRes = await dbClient.query('SELECT id, username, polygon_wallet, email FROM users WHERE lower(trim(email::text)) = $1 AND email = $2', [lower, trimmed]);
        ...
        // workshop_slots removida em 20260516180000_battery_uuids_and_purge_charging
        ...
        if (username)
            await dbClient.query('DELETE FROM wheel_players WHERE username = $1', [username]);
        if (wallet)
            await dbClient.query('DELETE FROM nft_items WHERE owner_address = $1', [wallet]);
...
app.post('/api/admin/bulk-gift', isAdmin, async (req, res) => {
    ...
    if (!Array.isArray(emails) || !gift)
        return res.status(400).json({ error: 'Emails and gift object required' });
    const { type, id, qty } = gift; // type: 'usdc' | 'item' | 'box' | 'coin'
    const amount = parseFloat(qty) || 0;
    if (amount <= 0 && type !== 'item')
...
function normalizeJsonSafeNumbers(value) {
    if (typeof value === 'bigint') {
        const num = Number(value);
        return (Number.isFinite(num) ? num : 0);
    }
    if (Array.isArray(value)) {
        return value.map((entry) => normalizeJsonSafeNumbers(entry));
    }
...
app.get('/api/game-state/:email', async (req, res) => {
    ...
    try {
        ...
        if (!progressRes.ok) {
            ...
            console.warn(`[GameState] computeProgress failed uid=${uid}: ${safeMsg} — serving snapshot without offline apply for this request`);
        }
        else {
        ...
        if (hadNftAutoViolations) {
            ...
            try {
                ...
                await fixClient.query('UPDATE game_states SET server_updated_at = $1, last_updated_at = $1 WHERE user_id = $2', [fixNow, uid]);
                ...
                for (const k of Object.keys(stock))
                    delete stock[k];
                for (const [k, v] of Object.entries(ch.stock))
                    stock[k] = v;
...
function dedupeStoredBatteriesByIdForGameState(list) {
    const out = [];
    const seen = new Set();
    for (const b of list) {
...
async function validatePlacedRacksForSave(dbq, racks, userId) {
    ...
    const ownedStoredBatteryIds = new Set();
    ...
    const isInfiniteBatteryCapacity = (pc) => pc != null && Number(pc) === -1;
    ...
    const fallbackBatteryCatalogId = String(finiteBatteriesSorted[0]?.id ?? catalogBatteryRows[0]?.id ?? '').trim();
    /** UUID de instância → id de catálogo vindo da rig (`placed_racks` / payload do save). */
    const placedRackBatteryCatalogByInstanceId = new Map();
    const ingestRackBatteryCatalogHint = (batteryIdRaw, catalogRaw) => {
        ...
        if (!cat || !catalogBatteryIds.has(cat))
            return;
        placedRackBatteryCatalogByInstanceId.set(bid, cat);
    };
    if (Number.isFinite(uidNum) && uidNum > 0) {
        try {
            const prCat = await dbq.query(`SELECT battery_id::text AS bid, battery_catalog_item_id::text AS cat
           FROM placed_racks
          WHERE user_id = $1::int
            AND battery_id IS NOT NULL
            AND battery_catalog_item_id IS NOT NULL`, [uidNum]);
            for (const row of prCat.rows || []) {
                ingestRackBatteryCatalogHint(row.bid, row.cat);
            }
        }
        catch (ePr) {
            console.warn('[validatePlacedRacksForSave] placed_racks battery catalog hints:', ePr instanceof Error ? ePr.message : String(ePr));
        }
    }
    if (Array.isArray(racks)) {
    ...
    const refreshOwnedStoredBatteryIds = async () => {
        ...
        try {
            ...
            for (const row of idRows.rows || []) {
                ...
                if (rid)
                    ownedStoredBatteryIds.add(rid);
            }
        }
        catch (e) {
            console.warn('[validatePlacedRacksForSave] owned stored_batteries ids:', e instanceof Error ? e.message : String(e));
        }
    };
    ...
    for (const r of racks) {
        if (!r || typeof r !== 'object')
            return { ok: false, error: 'Rig inválida.' };
        if (typeof r.id !== 'string' || !RACK_ID_RE.test(r.id))
            return { ok: false, error: 'ID de rig inválido.' };
        if (r.itemId != null && r.itemId !== '' && !RACK_ID_RE.test(String(r.itemId)))
        ...
        if (r.slots && r.slots.length > 128)
            return { ok: false, error: 'Demasiados slots de máquina.' };
        if (r.multiplierSlots != null && !Array.isArray(r.multiplierSlots))
            return { ok: false, error: 'Slots de multiplicador inválidos.' };
        if (r.multiplierSlots && r.multiplierSlots.length > 64)
            return { ok: false, error: 'Demasiados multiplicadores.' };
        if (r.itemId)
        ...
        if (r.batteryId != null && String(r.batteryId).trim() !== '') {
            ...
            if (ownedStoredBatteryIds.has(bidRaw)) {
                ...
                if (fromStore && catalogBatteryIds.has(fromStore)) {
                    ...
                    if (rcOwn && catalogBatteryIds.has(rcOwn))
                        upgradeIds.add(rcOwn);
                ...
                if (rc && catalogBatteryIds.has(rc))
                    upgradeIds.add(rc);
                else if (fallbackBatteryCatalogId)
                    upgradeIds.add(fallbackBatteryCatalogId);
                else {
                    return {
                        ok: false,
                        error: 'Referência de bateria inválida numa rig (dados desatualizados). Recarregue a página (F5) e tente de novo.'
                    };
                }
            }
            else {
        ...
        for (const s of r.slots || []) {
            if (!s)
                continue;
            if (!RACK_ID_RE.test(String(s)))
                return { ok: false, error: 'Peça inválida num slot.' };
        ...
        for (const s of r.multiplierSlots || []) {
            if (!s)
                continue;
            if (!RACK_ID_RE.test(String(s)))
                return { ok: false, error: 'Peça inválida num slot de multiplicador.' };
            upgradeIds.add(String(s));
        }
        if (r.selectedCoinId) {
            if (!RACK_ID_RE.test(String(r.selectedCoinId)))
                return { ok: false, error: 'Moeda selecionada inválida.' };
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const saveDomainRaw = String((req.headers && req.headers['x-game-save-domain']) || '')
                .trim()
                .toLowerCase();
            const saveDomain = saveDomainRaw === 'inventory' || saveDomainRaw === 'servers'
                ? saveDomainRaw
                : '';
            if (saveDomain) {
                if (effectiveAdminOverride) {
                ...
                // Domínio 'workshop' descontinuado em 20260516180000_battery_uuids_and_purge_charging.
        ...
        ON CONFLICT (user_id) DO UPDATE SET
          ...
          ON CONFLICT (user_id, box_id) DO UPDATE SET qty = EXCLUDED.qty`, [uid, bv.boxIds, bv.qtys]);
            ...
            let preMountBatterySnapSave = new Map();
            ...
            if (changes.storedBatteries) {
                ...
                changes.storedBatteries = sanitizeStoredBatteriesForSavePayload(changes.storedBatteries, changes.placedRacks);
                ...
                const batRm = await validateStoredBatteryWarehouseRemovalAllowed(client, uid, incomingIds, { placedRacks: changes.placedRacks }, effectiveAdminOverride);
                if (!batRm.ok) {
                ...
                if (changes.storedBatteries.length > 0) {
                    ...
                    // Bateria infinita por design (UUID em `id`); sem `current_charge`/`power_capacity_wh`/workshop_*.
                    await client.query(`
          ...
          ON CONFLICT (id) DO UPDATE SET
            ...
            image_url = COALESCE(NULLIF(BTRIM(EXCLUDED.image_url), ''), stored_batteries.image_url)`, [uid, bIds, bItemIds, bNames, bImgs]);
                }
            }
            if (changes.placedRacks) {
                const ts = new Date().toISOString();
                const prevRacksRes = await client.query(`SELECT id, item_id, wiring_id, battery_id, is_on, selected_coin_id,
         ...
         ORDER BY s.rack_id, s.slot_index`, [uid]);
                ...
                if (changes.placedRacks.length > 0) {
                    const catalogIdsForUpgradesSave = new Set();
                    for (const r of changes.placedRacks) {
          ...
          ON CONFLICT (id) DO UPDATE SET
            ...
            // changes.workshopSlots: payload ignorado em 20260516180000 (oficina/recarga descontinuadas).
          ...
          ON CONFLICT (user_id, box_id) DO NOTHING`, [uid, changes.claimedBoxes, Date.now()]);
            ...
            if (nftAutoSanitized && changes.placedRacks) {
                ...
                const batRows = await client.query('SELECT id, item_id FROM stored_batteries WHERE user_id = $1', [uid]);
        ...
        const savePayload = { ok: true, serverUpdatedAt: finalServerUpdatedAt };
...
app.post('/api/game/save-servers', async (req, res) => {
    ...
    const srvChanges = { lastLoadTime, placedRacks };
    if (stock !== undefined)
        srvChanges.stock = stock;
    if (storedBatteries !== undefined)
        srvChanges.storedBatteries = storedBatteries;
    req.body = {
        changes: srvChanges,
        adminOverride: false
    };
...
// /api/game/save-workshop removida em 20260516180000_battery_uuids_and_purge_charging.
// --- BACKUP SETTINGS API ---
    ...
    try {
        const scanRigs = async (c) => {
            ...
            for (const r of racks.rows) {
                const components = [];
                if (r.item_id)
                    components.push(r.item_id);
                if (r.wiring_id)
                    components.push(r.wiring_id);
                if (r.battery_id)
                    components.push(r.battery_id);
                const slots = await c.query('SELECT machine_item_id FROM rack_slots WHERE rack_id = $1', [r.id]);
                slots.rows.forEach(s => { if (s.machine_item_id)
                    components.push(s.machine_item_id); });
                const multi = await c.query('SELECT multiplier_item_id FROM rack_multiplier_slots WHERE rack_id = $1', [r.id]);
                multi.rows.forEach(m => { if (m.multiplier_item_id)
                    components.push(m.multiplier_item_id); });
...
app.post('/api/admin/impersonate', isAdmin, async (req, res) => {
    ...
    if (!sid || !adminId)
        return res.status(400).json({ error: 'Sessão necessária para personificação' });
    try {
...
app.post('/api/gemini/fortune', async (req, res) => {
    ...
    if (!genAI)
        return res.json({ text: "O destino é incerto, mas a sorte sorri para os corajosos." });
    try {
        const prompt = `
...
app.post('/api/exchange/sell', async (req, res) => {
    ...
    try {
        ...
        console.log('[ExchangeSell] userId=%s coinId=%s coinName=%s pct=%s soldAmount=%s grossUsdc=%s feeUsdc=%s netUsdc=%s', uid, cid, coinName, String(pct), result.soldAmount, result.grossUsdc.toFixed(8), result.feeUsdc.toFixed(8), result.netUsdc.toFixed(8));
